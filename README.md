...
- **Un rapport peut aussi servir à corriger l'outil.** La première relecture par une IA d'un vrai dossier a révélé deux erreurs de calcul (un slippage lu dans un champ peu fiable, un volume limité au pool principal) et une synthèse plus rassurante que ses données. Une deuxième relecture a corrigé la lecture de la liquidité (le pool principal porte l'état, le total est secondaire), ajouté un repère de taille d'ordre à 1 % du pool principal, retrouvé le créateur des tokens de launchpad par leur première transaction, et écarté les pages de cours des actualités. Tout est dans docs/05, A.2.
