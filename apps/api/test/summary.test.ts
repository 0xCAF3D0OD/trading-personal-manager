...
function base(): SummaryInputs {
  return {
    ...
    liquidity: { ratioPct: 12.4, band: 'comfortable', mainPoolUsd: 250_000, ts: now, source: 'dexscreener' },
...
describe('Synthèse en cinq questions', () => {
  it('donne cinq réponses dans l’ordre de lecture, jamais vides, sans total', () => {
    ...
    for (const x of a) { expect(x.answer.length).toBeGreaterThan(10); expect(x.short.length).toBeGreaterThan(0); }
  ...
  it('concentration, ventes de l’équipe et contradictions font basculer l’état', () => {
    ...
    i.liquidity = { ratioPct: 1.2, band: 'very_thin', mainPoolUsd: 4_000, ts: now, source: 'dexscreener' };
