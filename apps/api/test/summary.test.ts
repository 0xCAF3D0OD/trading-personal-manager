...
const settings = { summaryTop10ConcentratedPct: 40, summarySlippageOrderUsd: 1000, orderSizeShareOfPoolPct: 1 };
...
describe('Synthèse en cinq questions', () => {
  it('donne cinq réponses dans l’ordre de lecture, jamais vides, sans total', () => {
    ...
    expect(a[1]!.answer).toContain('dans le pool principal');
    expect(a[1]!.answer).toContain('Tous pools confondus (3) : 15,5 %');
    expect(a[1]!.answer).toMatch(/Repère de taille : 1 % du pool principal = 2.000 \$/);
    expect(a.map((x) => x.state)).toEqual(['ok', 'ok', 'ok', 'ok', 'ok']);
    expect(a[0]!.answer).toMatch(/^Non : l’équipe ne peut ni créer/);
    expect(a[1]!.answer).toMatch(/Vendre 1.000 \$ coûterait 0,42 %/); // espace fine insécable de fr-FR
  ...
  it('inconnu, partiel et non estimé sont dits avec leur raison', () => {
    ...
    i.divergences = { triggered: [], evaluated: 7, insufficient: 7, computedAt: now, priceChange24hPct: null, priceChangeAt: null };
  ...
  it('concentration, ventes de l’équipe et contradictions font basculer l’état', () => {
    ...
    i.liquidity = { ratioPct: 1.2, band: 'very_thin', mainPoolUsd: 4_000, totalRatioPct: 1.2, totalBand: 'very_thin', totalUsd: 4_000, poolsCount: 1, ts: now, source: 'dexscreener' };
    i.team = { ...i.team, sells: 3, claimsContradicted: 1, claimsKept: 0, claimsExpired: 0 };
    i.divergences = { triggered: ['Prix en hausse, volume en baisse', 'Retrait de liquidité'], evaluated: 7, insufficient: 0, computedAt: now, priceChange24hPct: -53, priceChangeAt: now };
