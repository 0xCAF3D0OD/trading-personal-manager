...
defineProps<{ rows: ScanResult[]; mode: 'kept' | 'excluded'; detail: boolean }>();
...
function cols(mode: 'kept' | 'excluded', detail: boolean): number { return (detail ? 9 : 5) + (mode === 'kept' ? 1 : 0) + 3; }
</script>
<template>
  <div class="overflow">
    <table>
      <thead>
        <tr>
          <th>Token</th><th class="num">Âge</th><th class="num">24 h</th>
          <template v-if="detail"><th class="num">1 h</th><th class="num">Capitalisation</th></template>
          <th class="num">Liquidité</th>
          <template v-if="detail"><th class="num">Volume 24 h</th><th class="num">Vol / cap</th></template>
          <th v-if="mode === 'kept'" class="num">Filtres</th><th>{{ mode === 'kept' ? 'Drapeaux' : 'Motif d’exclusion' }}</th>
          <th>Où l’acheter</th><th></th>
        </tr>
      </thead>
      <tbody>
        <template v-for="r in rows" :key="r.id">
          <tr style="cursor:pointer" @click="open = open === r.id ? null : r.id">
            <td><strong>{{ r.tokenSymbol ?? shortAddr(r.tokenAddress) }}</strong> <span class="faint small">{{ r.tokenName }}</span><div v-if="detail" class="mono faint small">{{ shortAddr(r.tokenAddress, 6) }} · {{ r.metrics.dexId }}</div></td>
            ...
            <td>
              <template v-if="mode === 'kept'">
                <span class="badge" :class="r.flagCount === 0 ? 'ok' : r.flagCount <= 2 ? 'warning' : 'risk'">{{ r.flagCount }} <Terme mot="drapeau">drapeau(x)</Terme></span>
            ...
            <td class="small">
              <span v-for="v in r.venues" :key="v.kind + v.identifier" class="badge" :class="v.isKraken ? 'ok' : 'neutral'" :title="v.kind === 'dex' ? 'DEX, connu par l’adresse du pool' : `Plateforme centralisée, via la fiche CoinGecko${v.volume24hUsd ? ` · ${fmtUsd(v.volume24hUsd, { compact: true })} / 24 h` : ''}`" style="margin:.1rem .2rem .1rem 0">
                <a v-if="v.url" :href="v.url" target="_blank" rel="noopener" style="color:inherit" @click.stop>{{ v.name }}</a><template v-else>{{ v.name }}</template>
              </span>
              <div v-if="r.venuesNote" class="faint" style="max-width:220px">{{ r.venuesNote }}</div>
            </td>
          ...
          <tr v-if="open === r.id">
            <td :colspan="cols(mode, detail)" style="background:var(--bg-elev-2)">
              ...
              <p class="small faint" style="margin:.5rem 0 0">
                ...
                <template v-if="!detail"> · capitalisation {{ fmtUsd(r.metrics.capUsd, { compact: true }) }}{{ r.metrics.mcapIsFdv ? ' (FDV, non vérifiée)' : '' }} · volume 24 h {{ fmtUsd(r.metrics.volumeH24Usd, { compact: true }) }}</template>
                ...
                · <span class="mono">{{ r.tokenAddress }}</span>
